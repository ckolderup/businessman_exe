...
@independence = %w{Fireworks Flag Independence Bonfire Baton-Twirl Parade Grill Cookout Stars Stripes Constitution United Democracy Explosion Sparkler Summer Firecracker July Patriot Picnic Hamburger Hotdog Streamer Sunburn Poolparty Union Victory Beer Watermelon Bang Boom Crackle Pie Spangle Liberty State Congressman Senator President Liberty Justice Baseball Taxation Yankee Colony Allegiance Freedom}
...
def computer_company
  company_front = %w[Elec Inter Macro Globo Hyper Infra] + @independence
...
def dumb
  ...
  last = %w[Meats Futures Industries Metals Investments Logging Infrastructure Instruction Development Research Systems] + @independence
...
def image(name, title, company)
  ...
  image = image.opaque('#55FFFF', 'red')
  image = image.opaque('#FF55FF', 'blue')
